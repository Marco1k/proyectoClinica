<nav class="bg-gray-800" x-data="{open: false}">
    <div class="max-w-7xl mx-auto px-2 sm:px-6 lg:px-8">
        <div class="relative flex items-center justify-between h-16">
            ...
            <div class="flex-1 flex items-center justify-center sm:items-stretch sm:justify-start">
                ...
                <a href="/" class="flex-shrink-0 flex items-center">
                    <img class="block lg:hidden h-8 w-auto" src="https://image.flaticon.com/icons/png/512/12/12426.png" alt="Noticias">
                    <img class="hidden lg:block h-8 w-auto" src="https://image.flaticon.com/icons/png/512/12/12426.png" alt="Noticias">
                </a>
                <div class="hidden sm:block sm:ml-6">
                    ...
                    <!-- Current: "bg-gray-900 text-white", Default: "text-gray-300 hover:bg-gray-700 hover:text-white" -->
                        ...
                        @foreach($categories as $category)
                            <a href="{{route('citas.category', $category)}}"
                ...
                <div class="absolute inset-y-0 right-0 flex items-center pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0">
                    ...
                    <div class="ml-3 relative" x-data="{open: false}">
                        ...
                        <div x-show="open" x-on:click.away="open = false" class="origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg py-1 bg-white ring-1 ring-black ring-opacity-5 focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="user-menu-button" tabindex="-1">
                            ...
                            <a href="{{ route('admin.informacion.create') }}" class="block px-4 py-2 text-sm text-gray-700" role="menuitem" tabindex="-1" id="user-menu-item-0"">Solicitud de información</a>
        ...
        @else
            <div>
                ...
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <a href="{{ route('admin.informacion.create') }}" class="text-gray-300 hover:bg-gray-700 hover:text-white px-3 py-2 rounded-md text-sm font-medium">Solicitud de información</a>
    ...
    <div class="sm:hidden" id="mobile-menu" x-show="open" x-on:click.away="open = false">
        <div class="px-2 pt-2 pb-3 space-y-1">
            ...
            @foreach($categories as $category)
                <a href="{{route('citas.category', $category)}}" class="text-gray-300 hover:bg-gray-700 hover:text-white px-3 py-2 rounded-md text-sm font-medium">{{$category->name}}</a>
            @endforeach
